...
@tree.command(name = "continents", description = "Get continent status", guild=discord.Object(id=873908844806434846)) #Add the guild ids in which the slash command will appear. If it should be in all, remove the argument, but note that it will take some time (up to an hour) to register the command if it's for all guilds.
async def first_command(interaction):
    continentsList = await main()
    #print("continentsList = ", continentsList)

    print("open_continents_list = ", continentsList)
    status = {
    ...
    title="Continent Status", description="Emerald (US-East)", color=0x5865F2, timestamp=discord.utils.utcnow())
    embedVar.add_field(name="Amerish", value=status["Amerish"], inline=False)
    embedVar.add_field(name="Esamir", value=status["Esamir"], inline=False)
    embedVar.add_field(name="Hossin", value=status["Hossin"], inline=False)
    embedVar.add_field(name="Indar", value=status["Indar"], inline=False)
    embedVar.add_field(name="Oshur", value=status["Oshur"], inline=False)
    embedVar.set_footer(text="Created by wupasscat")
